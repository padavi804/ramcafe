<?php
/**
 * Bloomerang self-hosted donation form (form ID 103424).
 *
...
<script type="text/javascript">
    var insertForm103424 = function() {
        var html103424 = '<style text=\'text/css\'>' + 
...
'<input id="fund" name="fund" value="10" maxlength="255"></input>' + 
'</div>' + 
'' + 
'</div>' + 
...
'<label for="consent-all"><input id="consent-all" name="consent-all" type="checkbox" maxlength="255"></input><span class="label">I would like to receive or continue receiving updates from Rivers Area Memory Cafe</span></label>' + 
'</div>' + 
...
'      <label id=\'noCaptchaResponseError\' class=\'error noCaptchaResponseError\' style=\'display: none\'>You must fill out the CAPTCHA</label><div id="captcha103424"></div>' + 
'' + 
'</div>' + 
...
'</div>' + 
'' + 
'' +  '';var successHtml103424 = '<div class=\'donation-success\'>' +
                                                       '  <h2>Thank You for Your Donation!</h2>' +
                                                       '  <p>Your generous gift has been processed. We truly appreciate your continued support.</p>' + 
                                                        '   ' + 
                                                        '</div>';( function($) {if (Bloomerang.useDonationId('103424')) { 
                                Bloomerang.useProcessor('12289', 'OnePay', 'pk_live_iZYXFefCkt380zu63aqUIo7y');
                            } else {
                                html103424 = '<p style="color: red">Only one donation or event registration form can be used on each page.</p>';
                            }if (jQuery('#bloomerangForm103424').length) {
                        
                    if (window.ActiveXObject) { // they are using IE < 11, which doesn't support TLS 1.1
                        html103424 = '<p style="color: red">Your browser does not support the minimum security requirements for keeping your Credit Card information safe when processing payments. Please upgrade your browser or download the latest version of' + 
                        ...
                        jQuery('#bloomerangForm103424').after(html103424);
                        
                        
                        Bloomerang.Util.requireStripe(function() {
                              Bloomerang.useKey('pub_a6332d51-665e-11f1-9ea1-06612d4c9559');
...
Bloomerang.Data.PayPal.IsPayPalPaymentMethodVaultingEnabled = false;Bloomerang.Data.PayPal.IsVenmoSameDayDonationEnabled = false;
                        ...
                        function initializeCaptcha() {
                            Bloomerang.Util.load('https://www.google.com/recaptcha/api.js?onload=captchaLoadCallback&render=explicit',
                                ...
                                function() {
                                    ...
                                    jQuery('form.donation-form').data('captcha-id', grecaptcha.render('captcha103424', { 'sitekey' : '6LeOuj4tAAAAANPyvtnBABskDXH_Y9YK8CVBVcQI' }));
        ...
        Bloomerang.Api.OnSuccess = Bloomerang.Widget.Donation.OnSuccess = function (response) {
            ...
            formContainer.show();
            formContainer.html(successHtml103424);
        ...
        Bloomerang.Util.updateDonateButtonText = function() {
            ...
            // PID-186: refresh the Stripe (Express) Payment Element wallet amount when the donor
            // changes the amount/fee, so the Apple/Google Pay sheet shows the correct total.
            else if (Bloomerang.UsePaymentElement && Bloomerang.Util.onDonationAmountChanged) {
